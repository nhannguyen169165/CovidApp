...
import { CountryDetailData,CountryChart } from "../../app/interfaces/country.interface";
...
>("country/fetchCountryChartAsync", async ({ slug }, thunkAPI) => {
  try {
    ...
    const to = moment(new Date(new Date().setDate(new Date().getDate() - 1))).format('YYYY-MM-DD')+"T00:00:00Z";
    const from = moment(new Date(new Date().setDate(new Date().getDate() - 30))).format('YYYY-MM-DD')+"T00:00:00Z";
